...
| **Factory + Registry** | Enregistrement, recherche et mise en cache centralisés des calculateurs, avec création paresseuse et double vérification sous verrou | `DefaultFactory` (`internal/fibonacci/registry.go`), consommé via `orchestration.CalculatorSource` et `app.CalculatorRegistry` — interfaces définies côté consommateur depuis le 2026-09-07 |
